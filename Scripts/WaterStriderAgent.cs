...
public class WaterStriderAgent : MonoBehaviour
{
    [SerializeField]
    public float _maxSpeed { get; private set; } = 0.05f;
    ...
    //// Hide 관련 변수
    [SerializeField]
    private Transform _pursuer = null;
    // 장애물로부터 멀어져 있을 거리
    [SerializeField]
    private float _distanceFromObstacle = 5.0f;
    ...
    {
        // 조종힘을 계산
        _velocity = _velocity + (Hide() + WallAvoidance() + ObstacleAvoidance() * Time.deltaTime);
    ...
    private Vector3 Hide()
    {
        if (_pursuer != null)
        {
            ...
            if (obstacles != null)
            {
                ...
                {
                    // 해당 장애물에서의 은신좌표를 계산
                    float distAway = (obstacles[i].GetComponent<SphereCollider>().radius * obstacles[i].transform.lossyScale.x) + _distanceFromObstacle;

                    Vector3 correctedObPos = obstacles[i].transform.position;
                    correctedObPos.y = transform.position.y;

                    Vector3 correctedPsPos = _pursuer.position;
                    correctedPsPos.y = transform.position.y;

                    Vector3 dirToOb = (correctedObPos - correctedPsPos).normalized;
                    Vector3 hidingSpot = (dirToOb * distAway) + correctedObPos;

                    // 가장 가까운 좌표를 기억
                    float dist = (transform.position - hidingSpot).sqrMagnitude;
                ...
                // 마땅한 좌표가 없는 경우(최단거리 초기값이 그대로인 경우) 도주 타겟으로부터 Evade를 반환.
                if (distToClosest == float.MaxValue)
                {
                    return Evade(_pursuer);
                }
            ...
            // 도주할 장애물이 없는 경우 도주 타겟으로부터 Evade를 반환.
            return Evade(_pursuer);
        }

        // 도주 타겟이 없는 경우 조종힘을 반환하지 않음.
        return Vector3.zero;
    }
