...
public class LeaderFishAgent : MonoBehaviour
{
    [SerializeField]
    public float _maxSpeed { get; private set; } = 0.02f;
    ...
    // 방황하는 정도의 보정치. 큰 값일수록 방향을 바꾸는 정도가 강해짐.
    [SerializeField]
    private float _wanderPower = 25.0f;
    ...
    void Start()
    {
        _wanderTarget = new Vector3(Mathf.Cos(Random.Range(0.0f, 2.0f) * Mathf.PI), 0, Mathf.Sin(Random.Range(0.0f, 2.0f) * Mathf.PI));
    }
    ...
    {
        // 1차적으로 방황 조종힘을 추가.
        _velocity = _velocity + ((Wander() + WallAvoidance() + ObstacleAvoidance()) * Time.deltaTime);
    ...
    {
        float wanderSize = _wanderPower * Time.deltaTime;
        _wanderTarget += new Vector3(Random.Range(-1.0f, 1.0f) * wanderSize, 0, Random.Range(-1.0f, 1.0f) * wanderSize);
        _wanderTarget.Normalize();
        _wanderTarget *= _wanderRadius;

        // 방황의 최종 목적지 = Agent의 현 위치 + (Agent의 전방으로 특정 거리만큼 이동한) 원 둘레 상의 특정 지점
        Vector3 target = transform.position + (_velocity.normalized * _wanderDistance) + _wanderTarget;
